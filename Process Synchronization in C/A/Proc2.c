...
int main() {
	...
	/*synchronize with Proc1, Proc3 and Proc4 (4 process 4 way sync.)*/ 
	...
	printf("\nWaiting for Semaphore1 signals to start Proc2");



	s_wait = sem_wait(sem1);
	s_wait = sem_wait(sem1);
	s_wait = sem_wait(sem1);
	printf("\nsem1: Semaphore wait for sem1 concluded, processes synced, initializing Proc2"); 
	/*---------------------------------------------------------------------*/
